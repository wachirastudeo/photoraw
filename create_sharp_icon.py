...
try:
    ...
    # Enhance sharpness of the original image before resizing
    ...
    for size in sizes:
        ...
        # Apply slight sharpening after resize to compensate for blur
        if size[0] <= 128:  # Apply extra sharpening for smaller sizes
            sharpener = ImageEnhance.Sharpness(resized)
            resized = sharpener.enhance(1.8)
            # Apply unsharp mask for even better clarity
            resized = resized.filter(ImageFilter.UnsharpMask(radius=1, percent=150, threshold=3))
        
        icon_images.append(resized)
        print(f"  Created sharp {size[0]}x{size[1]} version")
    ...
    print(f"\n✅ Created high-quality sharp {ico_path}")
    ...
    # Apply sharpening
    sharpener = ImageEnhance.Sharpness(icon_large)
    icon_large = sharpener.enhance(1.5)
    ...
    # Apply strong sharpening for small size
    sharpener = ImageEnhance.Sharpness(icon_small)
    icon_small = sharpener.enhance(2.0)
    icon_small = icon_small.filter(ImageFilter.UnsharpMask(radius=1, percent=150, threshold=3))
    ...
    print("\n🎉 All SHARP high-quality icon files created successfully!")
    print("\n📋 Summary:")
    print(f"   • icon.ico: {os.path.getsize(ico_path):,} bytes (multi-resolution, sharpened)")
    ...
    print("\n🔍 Tip: After installing, if icon still looks blurry:")
